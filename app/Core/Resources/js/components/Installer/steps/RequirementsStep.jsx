    ...
    return (
        <div className="space-y-6">
            <div className="space-y-3">
                <h3 className="font-medium text-sm text-zinc-500 uppercase">{__('Extensions PHP')}</h3>
            ...
            <div className="space-y-3">
                <h3 className="font-medium text-sm text-zinc-500 uppercase">{__('Permissions Dossiers')}</h3>
            ...
            <Button onClick={onNext} className="w-full cursor-pointer" disabled={!canProceed}>
                {canProceed ? __('Continuer') : __('Veuillez corriger les erreurs')}
            </Button>
        </div>
    );
}